...
class TunnelManager:
    ...
    def start(self, server_id, port):
        ...
        with self._lock:
            ...
            self._tunnels[server_id] = tunnel
            threading.Thread(
                target=self._reader, args=(tunnel,), daemon=True, name=f"bore-{server_id}"
            ).start()

        # give bore a moment to report its assigned address
        for _ in range(40):
    ...
    def _reader(self, tunnel):
        try:
            for line in tunnel.proc.stdout:
                ...
                elif not tunnel.address and re.search(r"(?i)error|refused|failed|denied", line):
